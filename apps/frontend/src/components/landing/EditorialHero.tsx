...
export function EditorialHero({
  ...
  return (
    ...
    >
      {/* Mobile: shorter crop — photo fills width without towering empty bands */}
      <div
        ...
        <div className="max-w-xl md:max-w-xl lg:max-w-xl lg:pt-2">
          <div className="flex flex-col justify-center">
            ...
            <form onSubmit={onSubmitSearch} className="mt-4 lg:mt-7" role="search" aria-label="Search catalog">
              <div className="flex w-full flex-col overflow-hidden rounded-xl border border-white/55 bg-white/92 shadow-sm backdrop-blur-md backdrop-saturate-150 transition-[border-color,box-shadow] duration-300 focus-within:border-white/85 focus-within:bg-white/97 focus-within:shadow-md sm:flex-row sm:items-stretch">
                <div className="flex min-h-[3rem] flex-1 items-center gap-3 px-3.5 sm:min-h-[3.125rem] sm:px-4 lg:min-h-[3.25rem]">
                ...
                <button
                  type="submit"
                  className="min-h-[3rem] w-full shrink-0 bg-[var(--brand-blue)] px-6 py-3 text-base font-semibold text-[#fafaf9] transition-colors duration-200 hover:bg-[var(--brand-blue-hover)] sm:w-auto sm:min-h-[3.125rem] sm:rounded-none sm:rounded-br-xl sm:rounded-tr-xl sm:px-8 lg:min-h-[3.25rem]"
                >
                  Search
                </button>
              </div>
