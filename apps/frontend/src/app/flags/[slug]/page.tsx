...
export default function FlagDetailPage() {
  ...
  // Group formats by category for tab counts
  const formatCounts = {
  ...
  return (
    <main style={{ padding: 'var(--spacing-xl) var(--spacing-lg)' }}>
      <div className="container" style={{ maxWidth: '1400px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-md)' }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 'var(--spacing-sm)', alignItems: 'center', flexWrap: 'wrap' }}>
                {flag.country_code && (
                  <span className="badge">{flag.country_code}</span>
                )}
                {flag.organization_name && (
                  <span className="badge">{flag.organization_name}</span>
                )}
                {flag.category && (
                  <span className="badge">{flag.category.name}</span>
                )}
                {isAdmin && (
                  <StatusBadge status={flag.status} isPremium={flag.is_premium} />
                )}
              </div>
            </div>
          </div>
        ...
        {/* Admin Controls */}
        {isAdmin && (
          <AdminControls asset={flag} onUpdate={loadFlagData} />
        )}
        ...
        <div className="grid" style={{ gridTemplateColumns: '1fr 400px', gap: 'var(--spacing-xl)' }}>
                    ...
                    {filteredFormats.map((asset: any) => {
                      ...
                      return (
                        <FormatCard
                          ...
                          onDownload={() => handleDownload(asset.id, asset.format_id, price.requires_subscription)}
                        />
                      );
                    })}
                  </div>
          ...
          <div>
            ...
            {showPremiumCTA && !hasPremium && (
              <PremiumCTA
                premiumFormatsCount={filteredFormats.filter((f: any) => f.price?.requires_subscription).length}
                onDismiss={() => setShowPremiumCTA(false)}
                variant="banner"
              />
            )}

            {/* Always show CTA if premium formats exist and user not subscribed */}
            {!hasPremium && filteredFormats.some((f: any) => f.price?.requires_subscription) && !showPremiumCTA && (
              <PremiumCTA
                premiumFormatsCount={filteredFormats.filter((f: any) => f.price?.requires_subscription).length}
                variant="banner"
              />
