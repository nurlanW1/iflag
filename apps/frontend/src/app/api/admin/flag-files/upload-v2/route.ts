...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    const backendRes = await fetchBackendApi(api.baseUrl, '/admin/flag-files/upload-v2', {
      method: 'POST',
      body: formData,
    ...
    console.error('[admin/flag-files/upload-v2] proxy failed:', err);
    return backendUnreachableResponse(api.baseUrl, '/admin/flag-files/upload-v2', err, 503);
  }
}
