...
/**
 * Proxies to Railway `POST /api/admin/import-r2` after Clerk admin gate.
 * Alias of `/api/admin/flag-files/import-r2` — same Clerk allow-list & R2 import.
 */
export async function POST(request: Request): Promise<Response> {
  ...
  const target = `${api.baseUrl}/admin/import-r2${qs ? `?${qs}` : ''}`;
  ...
  try {
    ...
    console.error('[admin/import-r2] proxy failed:', err);
