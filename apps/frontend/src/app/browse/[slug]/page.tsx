...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const canonical = `/browse/${product.slug}`;
...
export default async function BrowseProductPage({ params }: Props) {
  ...
  const seoPayload: AssetSeoPayload = {
    ...
    canonicalPath: `/browse/${product.slug}`,
    ...
    <>
      <JsonLd
        data={[
          ...
          breadcrumbJsonLd([
            ...
            { name: product.title, path: `/browse/${product.slug}` },
          ]),
        ]}
      />
      <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-10 lg:grid-cols-[1fr_minmax(0,24rem)]">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-[#009ab6]">
              {categoryName}
            </p>
              ...
              <ul className="mt-8 grid gap-4 sm:grid-cols-2">
                {uniquePreview.map((src, i) => (
                  ...
                  >
                    <Image
                      src={src}
                      alt={i === 0 ? `${product.title} preview` : `${product.title} preview ${i + 1}`}
                      fill
          ...
          <aside className="h-fit space-y-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm lg:sticky lg:top-6">
            ...
            {publicProduct.freeDownloadUrl ? (
              ...
              <p className="text-xs text-gray-500">
                No public preview file is linked for this product. Pro files are available to
                subscribers after purchase.
              </p>
            )}
            ...
            {paid ? (
              <div className="border-t border-gray-100 pt-6">
                ...
                <p className="mt-1 text-xs text-gray-600">
                  Buy a perpetual license for full-resolution files (checkout via Lemon Squeezy). Map
                  this product slug to a LS variant in{' '}
